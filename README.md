...
`id?: string` - отвечает за id товара.
...
`onInputChange(field: keyof T, value: string)` - метод необходимый для генерации события.
...
`set buttonText(value: string)` - сеттер, который необходим для установки текста кнопки у товара.
...
`set email(value: string)` - сеттер, который необходим для установки электронной почты пользователя.

`set phone(value: string)`` - сеттер, который необходим для установки номера телефона пользователя.

### Презентер

* Презентер (main.js) отвечает за основную логику приложения.

#### Обработка событий

* `catalog:change` - рендер карточек товаров после того, как в модели данных обновляется список сохраненных товаров.

* `card:click` - передает в модель данных id товара, на который нажали в каталоге.

* `card:choose` - рендер превью карточки товара, после того, как в модель данных попадает id товара, на который нажали в каталоге.

* `card__preview:click` - передает в модель корзины информацию о товаре после того, как пользователь нажмет на кнопку в превью товара, для покупки или удаления его из корзины.

* `basket:open` - рендер корзины в модальном окне при нажатии на кнопку корзины в шапке приложения.

* `basket:add-card` - меняет счётчик корзины при добавлени в неё товара.

* `card__basket:remove` - удаляет товар из корзины по id после нажатия на кнопку в корзине.

* `basket:remove-card` - рендерит обновленный список корзины после удаления товара из неё.

* `basket:success` - рендерит форму оформления заказа, после нажатие на кнопку "Оформить" в корзине.

* `form:change` - передает в модель данных о покупателе данные об изменениях в форме.

* `form:validate` - обрабатывает данные полученные из формы в модель данных.

* `order:submit` - рендерит вторую форму.

* `contacts:submit` - отправляет на сервер POST запрос с данными о товаре и покупателе.

* `success:agree` - закрывает модальное окно с подтверждением об оформленном товаре.